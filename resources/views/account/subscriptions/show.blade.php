...
@section('content')
<div class="mb-6">
    <a href="{{ route('account.subscriptions.index') }}" class="text-indigo-600 text-sm font-medium hover:underline">&larr; Back to subscriptions</a>
</div>

<x-card class="p-6 mb-6">
    <h1 class="text-2xl font-semibold text-slate-800 mb-2">{{ $subscription['product_title'] ?? 'Subscription' }}</h1>
...
<div class="space-y-4" x-data="subscriptionActions({{ $subscription['id'] }})">
    @if(!empty($enabledFeatures['enable_pause']))
        <x-card class="p-5">
            <h2 class="font-medium text-slate-800 mb-3">Pause / Resume</h2>
            <button @click="pause()" x-show="!loading" class="rounded-lg border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">Pause subscription</button>
            <button @click="resume()" x-show="!loading" class="rounded-lg border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 ml-2">Resume</button>
            <span x-show="loading" class="text-sm text-slate-500">Saving...</span>
        </x-card>
    @endif

    <x-card class="p-5">
        <h2 class="font-medium text-slate-800 mb-3">Change next charge date</h2>
        <form @submit.prevent="updateNextChargeDate()" class="flex flex-wrap gap-2 items-end">
            <input type="date" x-model="nextChargeDate" :min="minDate" class="rounded-lg border border-slate-300 px-3 py-2 text-sm">
            <button type="submit" class="rounded-lg bg-indigo-600 text-white px-4 py-2 text-sm font-medium" x-bind:disabled="loading">Update</button>
        </form>
    </x-card>

    <x-card class="p-5">
        <h2 class="font-medium text-slate-800 mb-3">Quantity</h2>
        <form @submit.prevent="updateQuantity()" class="flex flex-wrap gap-2 items-end">
            <input type="number" x-model.number="quantity" min="1" class="rounded-lg border border-slate-300 px-3 py-2 text-sm w-24">
            <button type="submit" class="rounded-lg bg-indigo-600 text-white px-4 py-2 text-sm font-medium" x-bind:disabled="loading">Update</button>
        </form>
    </x-card>
    ...
    @if(!empty($enabledFeatures['enable_cancel']))
        <x-card class="p-5">
            <h2 class="font-medium text-slate-800 mb-3">Cancel subscription</h2>
            <button @click="cancel()" x-show="!loading" class="rounded-lg border border-red-300 text-red-700 px-4 py-2 text-sm font-medium hover:bg-red-50">Cancel subscription</button>
            <span x-show="loading" class="text-sm text-slate-500">Saving...</span>
        </x-card>
    @endif
</div>
